...
let%expect_test "if_else" =
  Test_util.check_expr "if true then 1 else 0" Ast.Type.Num (Ast.Expr.Num 1);
  Test_util.check_expr "if false then 1 else 0" Ast.Type.Num (Ast.Expr.Num 0);
  Test_util.check_expr
