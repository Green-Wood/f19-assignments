...
let parse f input =
  ...
  | Grammar.Error ->
    ...
    Error
      (Printf.sprintf
         "Parse error: line %d, characters %d-%d"
         start_pos.pos_lnum
...
let parse_expr = parse Grammar.expr_toplevel
let parse_expr_exn = parse_exn Grammar.expr_toplevel
let parse_type_exn = parse_exn Grammar.type_toplevel
